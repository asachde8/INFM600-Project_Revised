...
#converting catagorical data of years Of Coding professionaly into numerical data
...
View(data4)
#converting catagorical data of Jobsatisfaction column into numerical data
...
data2$ConvertedSalary<-as.numeric(data2$ConvertedSalary)

plot(data2$JobSatisfaction,data2$ConvertedSalary,xlab="Job Satisfaction" ,ylab="Salary in U.S. Dollars")
#perfor correlation test to see if the two entities are correlated or not
cor.test(data2$JobSatisfactionNumeric,data2$ConvertedSalary)

#question 2
#similar to the code above. Plotting the two variables together
plot(data2$ConvertedSalary,data2$Country, las =2)
...
plot(data2$JobSatisfaction,data2$YearsCodingProf,Xlab="Job Satisfaction",ylab="Years of Coding professionally")
...
langknown = data2$NumberOfLanguagesKnown
...
#question 4
#converting catagorical data of Last NewJob column into numerical data
#there are a lot of countries so it fills up the x-\else(data2$LastNewJob=="I've never had a job",0,
plot(data2$LastNewJob,data2$ConvertedSalary,xlab="Last Job Switch",ylab="Salary in U.S. Dollars")                                                   
data2$LastNewJob<-  ifelse(data2$LastNewJob=="I've never had a job",0,
                           ifelse(data2$LastNewJob=="Less than a year ago",1,
                                  ifelse(data2$LastNewJob=="Between 1 and 2 years ago",2,
...
plot(data2$LastNewJob,data2$YearsCodingProf,xlab="Last Switched job",ylab="Years of Working Professionally")
class(data2$YearsCodingProf)
#perfor correlation test to see if the two entities are correlated or not
cor.test(data2$LastNewJob,data2$YearsCodingProf)

#plot and correlation test for last switch and the present salary 
plot(data2$LastNewJob,data2$JobSatisfaction)
class(data2$ConvertedSalary)
#perfor correlation test to see if the two entities are correlated or not
...
View(data2$Age)
plot(data2$LastNewJob,data2$Age,ylab="Age",xlab="Job Search Status",las=1) 
#perfor correlation test to see if the two entities are correlated or not
...
plot(data2$StackOverflowJobs,xlab="Looking for Jobs using stackoverflow",ylab="Number of People")
